...
import { isSaturated, scoreTriple, selectModelTier, } from "./scoring.js";
...
export async function runImprovePass(input) {
    const { masterResume, currentResume, jdText = "", targetRole = "", version, focus = "balanced", matchScore, factLedger, complete, includeCover = false, profileJson, fitScoreJson, } = input;
    ...
    const messages = [
        { role: "system", content: tailorSystemPrompt(version, focus) },
...
export async function runImproveChain(input) {
    ...
    for (let v = 1; v <= input.targetVersion; v = (v + 1)) {
...
export async function runImproveMore(input) {
    const next = Math.min(4, input.currentVersion + 1);
