...
import { scoreTriple, deliverVersionForMatch, selectModelTier, buildFactLedger, validateFacts, sanitizeAtsMarkdown, normalizeCoverLetterMarkdown, } from "./index.js";
...
`;
    ...
    assert.ok(!s.missingKeywords.some((k) => k.includes("founditgulf")), "URL tokens should not appear as missing keywords");
});
...
test("fixDuplicatedFragments repairs AWAWS and AyAyub", () => {
    const out = normalizeCoverLetterMarkdown("Experience with AWAWS and signed by AyAyub Shaik.");
