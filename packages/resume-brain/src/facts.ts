...
export function buildFactLedger(masterResume: string): FactLedger {
  ...
  for (const line of text.split("\n")) {
    ...
    if (h3) employers.push(h3[1].trim());
...
export function validateFacts(
  ...
  const newYears = [...new Set(tailoredResume.match(YEAR_RE) || [])].filter(
    (y) => !ledger.years.includes(y) && !masterLower.includes(y),
  );
  if (newYears.length) {
    violations.push(`New years not in master: ${newYears.join(", ")}`);
  }

  const newMetrics = [...new Set((tailoredResume.match(METRIC_RE) || []).map((m) => m.toLowerCase()))].filter(
    (m) => !ledger.metrics.includes(m) && !masterLower.includes(m),
  );
