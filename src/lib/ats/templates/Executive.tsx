...
const s = StyleSheet.create({
  page: { paddingTop: 36, paddingBottom: 36, paddingHorizontal: 40, fontSize: 10, fontFamily: "Times-Roman", color: "#111" },
  ...
  title: { fontFamily: "Times-Bold", fontSize: 10 },
  meta: { fontSize: 9, color: "#444" },
  bullet: { marginLeft: 10, marginTop: 2, lineHeight: 1.3 },
  skill: { marginBottom: 3, lineHeight: 1.3 },
});

export function ExecutiveDocument({ data }: { data: NormalizedResume }) {
  const r = data;
  const b = r.basics;

  return (
    <Document title={`${b.name || "Resume"} — Executive`} author={b.name}>
