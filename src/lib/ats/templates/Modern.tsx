...
const s = StyleSheet.create({
  ...
  header: { marginBottom: 14, borderBottomWidth: 2, borderBottomColor: "#0f766e", paddingBottom: 10 },
  name: { fontSize: 22, fontFamily: "Helvetica-Bold", color: "#0f172a", marginBottom: 3 },
  label: { fontSize: 11, color: "#0f766e", marginBottom: 4 },
  contact: { fontSize: 9, color: "#334155" },
  section: { marginTop: 12 },
  heading: { fontSize: 10, fontFamily: "Helvetica-Bold", color: "#0f766e", letterSpacing: 1, textTransform: "uppercase", marginBottom: 6 },
  summary: { lineHeight: 1.4, color: "#1e293b" },
  item: { marginBottom: 9 },
  row: { flexDirection: "row", justifyContent: "space-between", gap: 8 },
  title: { fontFamily: "Helvetica-Bold", fontSize: 10.5 },
  company: { fontSize: 9.5, color: "#475569", marginTop: 1 },
...
export function ModernDocument({ data }: { data: NormalizedResume }) {
  const r = data;
  const b = r.basics;

  return (
    <Document title={`${b.name || "Resume"} — Modern`} author={b.name}>
          ...
          <View style={s.section}>
            ...
            {(r.work || []).map((w, i) => (
              <View key={i} style={s.item} wrap={false}>
                ...
                <Text style={s.company}>{[w.name, w.location].filter(Boolean).join(" · ")}</Text>
          ...
          <View style={s.section}>
            ...
            {(r.education || []).map((e, i) => (
              <View key={i} style={s.item} wrap={false}>
                ...
                <Text style={s.company}>{[e.studyType, e.area].filter(Boolean).join(" · ")}</Text>
              </View>
            ))}
          </View>
        ) : null}
