...
const s = StyleSheet.create({
  page: { paddingTop: 30, paddingBottom: 28, paddingHorizontal: 34, fontSize: 9, fontFamily: "Courier", color: "#111" },
  name: { fontSize: 15, fontFamily: "Courier-Bold", marginBottom: 2 },
  ...
  title: { fontFamily: "Courier-Bold", fontSize: 9 },
  meta: { fontSize: 8, color: "#555" },
  bullet: { marginLeft: 8, marginTop: 1, lineHeight: 1.25 },
  skill: { marginBottom: 2, lineHeight: 1.25 },
});

export function MonoDocument({ data }: { data: NormalizedResume }) {
  const r = data;
  const b = r.basics;

  return (
    <Document title={`${b.name || "Resume"} — Mono`} author={b.name}>
