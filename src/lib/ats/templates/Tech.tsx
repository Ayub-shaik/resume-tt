...
const s = StyleSheet.create({
  page: { paddingTop: 36, paddingBottom: 32, paddingHorizontal: 40, fontSize: 10, fontFamily: "Courier", color: "#0b1220" },
  name: { fontSize: 17, fontFamily: "Courier-Bold", marginBottom: 3, color: "#0ea5e9" },
  contact: { fontSize: 8, color: "#334155", marginBottom: 10 },
  ...
  title: { fontFamily: "Courier-Bold", fontSize: 10 },
  meta: { fontSize: 8, color: "#64748b" },
  bullet: { marginLeft: 10, marginTop: 2, lineHeight: 1.3 },
  skill: { marginBottom: 3, lineHeight: 1.3 },
});

export function TechDocument({ data }: { data: NormalizedResume }) {
  const r = data;
  const b = r.basics;

  return (
    <Document title={`${b.name || "Resume"} — Tech`} author={b.name}>
